...
class Vertex {
    ...
    void addEdge(Vertex<T> *dest, string AC, double w);
...
class Edge {
    ...
    string airline_code;
public:
    Edge(Vertex<T> *d,string AC, double w);
    Vertex<T> *getDest() const;
    void setDest(Vertex<T> *dest);
    double getDistance() const;
    void setAirline_code(string AC);
...
template<class T>
void Edge<T>::setAirline_code(string AC) {
 ...
 */
template <class T>
bool Graph<T>::addEdge(const T &sourc, const T &dest, string AC,double w) {
 ...
 */
template <class T>
void Vertex<T>::addEdge(Vertex<T> *d,string AC, double w) {
    adj.push_back(Edge<T>(d,AC, w));
}
...
vector<T> Graph<T>::topsort() const {
    ...
    while(!vertices.empty()){
        ...
        for (auto neighbor : current->adj) {
            neighbor.dest->indegree--;
            if (neighbor.dest->indegree == 0) {
                vertices.push(neighbor.dest);
            }
        }
    }
